...
- Python 3.8+
- 必須: Pillow
- 推奨: Ghostscript（PDF高品質圧縮）, PyMuPDF・pikepdf（ネイティブPDF処理）
...
4. 圧縮設定を行います（PDFエンジン選択、必要に応じてPDF品質スライダー、画像品質、リサイズ）。
...
- `backend/core/compressor_utils.py`: バックエンド公開APIの実装本体
...
- [ ] 入力欄へファイルまたはフォルダをD&Dしたとき、入力フォルダが更新される（D&D有効環境）。
