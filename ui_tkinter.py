...
class App(tk.Tk if not DND_AVAILABLE else TkinterDnD.Tk):
    ...
    def _build_folder_section(self):
        ...
        ttk.Button(row_in, text="選択", command=self.choose_input).pack(side='left', padx=2)
        tk.Button(row_in, text="クリーンアップ", command=self.cleanup_input,
                  bg='#d0f6ff').pack(side='left', padx=2)

        if DND_AVAILABLE:
            self.input_entry.drop_target_register(DND_FILES)
            self.input_entry.dnd_bind('<<Drop>>', self._on_drop_input)
            ttk.Label(row_in, text="（D&D可）", foreground="gray").pack(side='left', padx=3)
        else:
        ...
        ttk.Entry(row_out, textvariable=self.output_dir, width=45).pack(side='left', padx=5)
        ttk.Button(row_out, text="選択", command=self.choose_output).pack(side='left', padx=2)
        tk.Button(row_out, text="クリーンアップ", command=self.cleanup_output,
                  bg='#ffcaca').pack(side='left', padx=2)

    def _build_notebook(self):
        """タブウィジェット（圧縮設定 / ログ・進捗）を構築。"""
        ...
        self.notebook.add(self.log_tab, text=" ログ / 進捗 ")
    ...
    def _build_settings_tab(self):
        """圧縮設定タブの 4 セクション。"""
        self._build_pdf_section(self.settings_tab)
        self._build_image_section(self.settings_tab)
        self._build_resize_section(self.settings_tab)
    ...
    def _build_pdf_section(self, parent):
        ...
        # ── エンジン選択 ──
        engine_frame = ttk.Frame(pdf_lf)
        engine_frame.pack(fill='x', padx=5, pady=(4, 2))
        ...
        # ── ネイティブエンジンフレーム ──
        self.native_frame = ttk.Frame(pdf_lf)
        self._build_native_controls(self.native_frame)

        # ── GS エンジンフレーム ──
        self.gs_frame = ttk.Frame(pdf_lf)
    ...
    def _build_native_controls(self, parent):
        ...
        self.lossy_lf.pack(fill='x', padx=5, pady=2)
        ...
        self.native_lossless_lf.pack(fill='x', padx=5, pady=2)
    ...
    def _build_gs_controls(self, parent):
        ...
        preset_lf.pack(fill='x', padx=5, pady=2)
        ...
        ll_check_row.pack(fill='x', padx=10, pady=2)
        ...
        self.gs_lossless_lf.pack(fill='x', padx=5, pady=2)
    ...
    def _build_image_section(self, parent):
        ...
        img_lf.pack(fill='x', padx=5, pady=3)
    ...
    def _build_resize_section(self, parent):
        ...
        resize_lf.pack(fill='x', padx=5, pady=3)
    ...
    def _build_output_section(self, parent):
        ...
        out_lf.pack(fill='x', padx=5, pady=3)
    ...
    def _build_log_tab(self):
        ...
        self.progress = ttk.Progressbar(
            self.log_tab, maximum=100, length=650, mode='determinate')
    ...
    def _build_action_buttons(self):
        ...
        tk.Button(inner, text="圧縮開始", command=self.start_compress,
                  width=16, bg='#ccffcc').pack(side='left', padx=10)
        tk.Button(inner, text="終了", command=self.on_exit,
                  width=12, bg='#e6e6e6').pack(side='left', padx=10)
