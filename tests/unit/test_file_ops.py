...
def test_cleanup_folder_removes_zip_and_known_output_files(tmp_path: Path) -> None:
    ...
    assert any(message.startswith(tlog('ja', 'cleanup_deleted', path='')) for message in logs)
    assert any('bundle.zip' in message for message in logs)
    assert any('log.csv' in message for message in logs)


def test_cleanup_folder_logs_english_when_requested(tmp_path: Path) -> None:
    output_dir = tmp_path / 'output'
    output_dir.mkdir(parents=True, exist_ok=True)
    target_path = output_dir / 'remove.csv'
    ...
    assert any('Output folder cleanup complete' in message for message in logs)